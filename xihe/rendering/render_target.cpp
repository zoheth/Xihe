    ...
    device_{images.front().get_device()}, images_{std::move(images)}
{
	...
	auto it = std::ranges::find_if(images, [](backend::Image const &image) { return image.get_type() != vk::ImageType::e2D; });
	if (it != images.end())
	{
		throw VulkanException{VK_ERROR_INITIALIZATION_FAILED, "Image type is not 2D"};
	}

	extent_.width  = images.front().get_extent().width;
	extent_.height = images.front().get_extent().height;
